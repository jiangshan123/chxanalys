        ...
        for sub_seq in range(  0, sub_num   ):   
            ...
            if imgs !=0:            
                ...
                for nconf, seg_mask in enumerate( [seg_mask_v, seg_mask_p  ]):
                    ...
                    if not para_run:                         
                        g2v, lag_stepsv  =cal_g2( FD,  seg_mask, bad_frame_list,good_start, num_buf = 8, 
                                                 ) 
                    else:
                        g2v, lag_stepsv  =cal_g2p( FD,  seg_mask, bad_frame_list,good_start, num_buf = 8, 
                    ...
                    save_g2( g2v, taus=tausv, qr=rcen, qz=acen, uid=uid +'_1a_mq%s'%conf, path= data_dir_ ) 
                    if nconf==0:
                        g2s[run_seq + 1][i]['v'] = g2v
                    else:
                        g2s[run_seq + 1][i]['p'] = g2v
                    ...
                    if fit:
                        ...
                        save_g2( g2v_fit, taus=tausv_fit,qr=rcen, qz=acen, 
                                uid=uid +'_1a_mq%s'%conf+'_fit', path= data_dir_ )

                        res_pargsv_fit = dict(taus=tausv, q_ring_center= np.unique(rcen), 
                        ...
                        dfv = save_g2_fit_para_tocsv(g2v_fit_result, 
